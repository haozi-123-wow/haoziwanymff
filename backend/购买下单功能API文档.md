...
**订单状态流转**:
```
pending(待支付) → paying(正在支付) → paid(已支付) → completed(已完成)
     ↓
...
**注意事项**:
- 只能取消状态为 `pending`(待支付) 或 `paying`(正在支付) 的订单
- `paying` 状态的订单取消后，如果支付回调到达会拒绝处理
- 已支付的订单无法取消，如需退款请联系客服
...
- 支付请求发起后订单状态变为 `paying`(正在支付)
- 支付成功后订单状态变为 `paid`(已支付),并自动激活用户套餐
- `paying` 状态的订单在支付结果返回前不能再次发起支付
